...
// RulesFS holds the embedded rules filesystem
// This will be set by the rules package
var RulesFS embed.FS

// LoadRulesFromFS loads all rules from the embedded filesystem
func LoadRulesFromFS(rulesFS embed.FS) (map[string][]*SigmaRule, int, error) {
	rulesByCategory := make(map[string][]*SigmaRule)
	totalRules := 0

	// First try to read _index.json for available categories
	indexData, err := rulesFS.ReadFile("_index.json")
	if err == nil {
		// Use index to load specific categories
		var index IndexFile
		if err := json.Unmarshal(indexData, &index); err != nil {
			return nil, 0, fmt.Errorf("failed to parse _index.json: %w", err)
		}

		for category, entry := range index.Files {
			rules, err := loadRuleFile(rulesFS, entry.Path)
			if err != nil {
		...
		err = fs.WalkDir(rulesFS, ".", func(path string, d fs.DirEntry, err error) error {
			...
			if d.IsDir() || !strings.HasSuffix(path, ".json") || path == "_index.json" {
				return nil
			}

			rules, err := loadRuleFile(rulesFS, path)
			if err != nil {
				return nil // Skip errors
			}

			// Extract category from filename
			category := strings.TrimSuffix(filepath.Base(path), ".json")
			rulesByCategory[category] = rules
			totalRules += len(rules)
		...
		if err != nil {
			return nil, 0, fmt.Errorf("failed to walk rules directory: %w", err)
		}
	}

	return rulesByCategory, totalRules, nil
}

// loadRuleFile loads rules from a single JSON file
func loadRuleFile(rulesFS embed.FS, path string) ([]*SigmaRule, error) {
	...
	// Convert to pointer slice for efficiency
	rules := make([]*SigmaRule, len(ruleFile.Rules))
	for i := range ruleFile.Rules {
		rules[i] = &ruleFile.Rules[i]
	}

	return rules, nil
}

// LoadRulesFromBytes loads rules from a byte slice (for testing)
func LoadRulesFromBytes(data []byte) ([]*SigmaRule, error) {
	var ruleFile RuleFile
	if err := json.Unmarshal(data, &ruleFile); err != nil {
